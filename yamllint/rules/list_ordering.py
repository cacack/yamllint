...
def check(conf, token, prev, next, nextnext, context):
    ...
    if isinstance(token, (yaml.BlockMappingStartToken,
        ...
        context['stack'].pop()
    elif (isinstance(token, yaml.ScalarToken) and
          isinstance(next, yaml.ScalarToken)):
        if len(context['stack']) > 0 and context['stack'][-1].type == SEQ:
            if any(strcoll(next.value, item) < 0
                   for item in context['stack'][-1].items):
                yield LintProblem(
                    next.start_mark.line + 1, next.start_mark.column + 1,
                    'wrong ordering of item "%s" in sequence' % next.value)
            else:
                context['stack'][-1].items.append(next.value)
